...
export function analyzeUnicode(text: string, lines: string[]): UnicodeAnalysis {
  ...
  const hiddenCharsTotal = zwspCount + nbspCount + bomCount + ltrCount + rtlCount + wjCount + orcCount + softHyphenCount + thinSpaceCount + controlCharsCount;
  const hiddenCharsDetails: string[] = [];
  if (zwspCount > 0) hiddenCharsDetails.push(`${zwspCount} Zero-Width Space(s) (U+200B)`);
  // ZWJ/ZWNJ are intentionally treated as protected characters.
  // They are meaningful in emoji sequences and complex-script text.
  ...
  // Leading indentation is not a Unicode error. It can be intentional in code,
  // lists, poetry, and formatted documents, so it must not reduce the health score.
  const unicodeProblemsTotal = curlyDoubleCount + curlySingleCount + longDashCount + mixedLineEndingsCount + repeatedPunctuationCount + htmlEntityCount;
